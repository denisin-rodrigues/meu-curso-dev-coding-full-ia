...
interface FlipCardContextValue {
  isFlipped: boolean
  flipDirection: FlipDirection
  // `| undefined` explícito: exigido pelo exactOptionalPropertyTypes do projeto,
  // já que `disabled` desce das props como `boolean | undefined`.
    ...
    ({ className, style, ...props }, ref) => {
      ...
      return (
        <motion.div
          ...
          // `style` é extraído na desestruturação (acima) para não voltar pelo
          // `{...props}` como `MotionStyle | undefined`. O objeto mesclado é sempre
          // definido em runtime, então castamos para NonNullable e satisfazemos o
          // exactOptionalPropertyTypes do projeto.
          style={
            {
              ...TRANSFORM_STYLES,
              ...(style ?? {}),
            } as NonNullable<HTMLMotionProps<"div">["style"]>
          }
          {...props}
        />
      )
