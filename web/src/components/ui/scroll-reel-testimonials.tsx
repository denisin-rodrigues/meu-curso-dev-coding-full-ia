...
function Featured({ src, alt }: { src: string; alt?: string | undefined }) {
  return (
    <div
...
export function ScrollReelTestimonials({
  ...
  const middleItems = React.useMemo(() => {
    const items: Array<
      { type: 'cell' } | { type: 'featured'; t: ScrollReelTestimonial }
    > = []
    for (let i = 0; i < 3; i++) items.push({ type: 'cell' })
    testimonials.forEach((t, i) => {
      items.push({ type: 'featured', t })
      if (i < count - 1) {
  ...
  /* Sem depoimentos não há o que renderizar (e `current` seria undefined
        ...
        <div className='absolute inset-0 flex items-center justify-center gap-2'>
          ...
          >
            {middleItems.map((item, i) =>
              item.type === 'featured' ? (
                <Featured key={i} src={item.t.image} alt={item.t.alt} />
              ) : (
                <Cell key={i} />
              ),
            )}
